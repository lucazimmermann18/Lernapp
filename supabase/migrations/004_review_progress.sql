-- Per-word spaced-repetition state, protected by the same owner-only RLS rules.
create table if not exists public."reviewProgress" (
...
alter table public."reviewProgress" enable row level security;
drop policy if exists "Families manage own review progress" on public."reviewProgress";
create policy "Families manage own review progress" on public."reviewProgress" for all to authenticated
...
begin
 ...
 foreach table_name in array array['units','attempts','rewards','settings','achievements','reviewProgress'] loop
