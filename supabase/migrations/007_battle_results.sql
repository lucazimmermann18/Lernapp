-- Persistent vocabulary-battle results, private to the signed-in family.
create table if not exists public."battleResults" (
...
alter table public."battleResults" enable row level security;
drop policy if exists "Families manage own battle results" on public."battleResults";
create policy "Families manage own battle results" on public."battleResults" for all to authenticated
...
begin
 ...
 foreach table_name in array array['units','attempts','rewards','settings','achievements','reviewProgress','masteryTests','stories','battleResults'] loop
