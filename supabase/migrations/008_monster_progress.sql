-- Three-step error-monster progress, private to the signed-in family.
create table if not exists public."monsterProgress" (
...
alter table public."monsterProgress" enable row level security;
drop policy if exists "Families manage own monster progress" on public."monsterProgress";
create policy "Families manage own monster progress" on public."monsterProgress" for all to authenticated
...
begin
 ...
 foreach table_name in array array['units','attempts','rewards','settings','achievements','reviewProgress','masteryTests','stories','battleResults','monsterProgress'] loop
