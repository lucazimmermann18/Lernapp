-- Long-term mastery-test results are immutable learning records owned by one family.
create table if not exists public."masteryTests" (
...
alter table public."masteryTests" enable row level security;
drop policy if exists "Families manage own mastery tests" on public."masteryTests";
create policy "Families manage own mastery tests" on public."masteryTests" for all to authenticated
...
begin
 ...
 foreach table_name in array array['units','attempts','rewards','settings','achievements','reviewProgress','masteryTests'] loop
