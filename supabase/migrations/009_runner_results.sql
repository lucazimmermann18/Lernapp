-- Word Runner results, private to the signed-in family.
create table if not exists public."runnerResults" (
...
alter table public."runnerResults" enable row level security;
drop policy if exists "Families manage own runner results" on public."runnerResults";
create policy "Families manage own runner results" on public."runnerResults" for all to authenticated
...
begin
 ...
 foreach table_name in array array['units','attempts','rewards','settings','achievements','reviewProgress','masteryTests','stories','battleResults','monsterProgress','runnerResults'] loop
