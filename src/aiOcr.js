...
export async function recognizeVocabularyWithAI(file,columnOrder='en-de',provider='auto'){
 ...
 let response;
 try{
  response=await fetch(`${supabaseUrl}/functions/v1/extract-vocabulary`,{method:'POST',headers:getCloudRequestHeaders(token),body:JSON.stringify({image:await toBase64(file),mediaType:file.type,columnOrder,provider})});
 }catch(error){
  throw new Error('Die KI-Erkennung ist gerade nicht erreichbar. Die App versucht automatisch die lokale Erkennung.',{cause:error});
 }
